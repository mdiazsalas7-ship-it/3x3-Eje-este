    ...
    <div style={{padding:"14px 14px 80px"}}>
      ...
      ag.map(g=>{
        ...
        return(<div key={g} style={{...S.card,borderTop:`3px solid ${GRUPO_COLORS[g]}`,marginBottom:14}}>
          ...
          {gMatches.length===0?<div style={{padding:20,textAlign:"center",color:"#94a3b8",fontSize:"0.9rem"}}>Presiona GENERAR ({gt.length} equipos)</div>:
          <div>
            {fin.length>0&&<div style={{padding:"4px 12px",fontSize:"0.84rem",fontWeight:900,color:"#10b981",letterSpacing:1,fontFamily:F.bar}}>FINALIZADOS ({fin.length})</div>}
            {fin.map(m=>{const lT=allTeams.find(t=>t.id===m.localId);const vT=allTeams.find(t=>t.id===m.visitId);return(
              <div key={m.id} style={{padding:"14px 16px",borderBottom:"1px solid #f1f5f9",display:"flex",alignItems:"center",gap:6}}>
                <div style={{width:8,height:8,borderRadius:"50%",background:"#10b981",flexShrink:0}}/>
                <span style={{flex:1,fontWeight:700,fontSize:"0.9rem",color:"#1e293b"}}>{lT?.nombre||"?"}</span>
                <span style={{fontWeight:900,color:"#1e293b",fontSize:"1rem",padding:"2px 8px",background:"#f1f5f9",borderRadius:5}}>{m.marcadorL||0} - {m.marcadorV||0}</span>
                <span style={{flex:1,fontWeight:700,fontSize:"0.9rem",color:"#1e293b",textAlign:"right"}}>{vT?.nombre||"?"}</span>
              </div>
            );})}
            {prog.length>0&&<div style={{padding:"4px 12px",fontSize:"0.84rem",fontWeight:900,color:"#f59e0b",letterSpacing:1,fontFamily:F.bar,marginTop:fin.length?4:0}}>PENDIENTES ({prog.length})</div>}
            {prog.map(m=>{const lT=allTeams.find(t=>t.id===m.localId);const vT=allTeams.find(t=>t.id===m.visitId);return(
              <div key={m.id} style={{padding:"14px 16px",borderBottom:"1px solid #f1f5f9",display:"flex",alignItems:"center",gap:6}}>
                <div style={{width:8,height:8,borderRadius:"50%",background:"#f59e0b",flexShrink:0}}/>
                <span style={{flex:1,fontWeight:700,fontSize:"0.9rem",color:"#64748b"}}>{lT?.nombre||"?"}</span>
                <span style={{fontSize:"0.95rem",color:"#94a3b8",fontWeight:700}}>VS</span>
                <span style={{flex:1,fontWeight:700,fontSize:"0.9rem",color:"#64748b",textAlign:"right"}}>{vT?.nombre||"?"}</span>
                <button onClick={()=>{onIniciarPartido(m);setMesaStep("active");onNav("mesa");}} style={{background:"#7c3aed",color:"white",border:"none",padding:"4px 8px",borderRadius:5,fontWeight:900,fontSize:"0.88rem",cursor:"pointer",flexShrink:0}}>▶️</button>
              </div>
            );})}
          </div>}
        </div>);
...
function MesaView({step,teams,players,mesaMatch,matches,statsPartido,allJugadas,onCrearPartido,onStat,onDeletePlay,onFinalizar,onBack,setMesaStep,setMesaMatch,genColor}){const[localId,setLocalId]=useState("");const[visitId,setVisitId]=useState("");const[showHist,setShowHist]=useState(false);const[confirmFin,setConfirmFin]=useState(false);const[delConfirm,setDelConfirm]=useState(null);
  if(step==="select"){const ec=matches.filter(m=>m.estatus==="programado");return(<div style={S.page}><Hdr title="⏱️ MESA TÉCNICA" onBack={onBack} color={genColor}/><div style={{padding:16,maxWidth:460,margin:"0 auto",width:"100%"}}>{teams.length<2?<Empty icon="⏱️" title="2+ equipos requeridos" desc="Registra equipos primero"/>:<div style={{...S.card,padding:14}}><p style={{fontFamily:F.bar,fontWeight:700,fontSize:"1rem",color:"#1e293b",marginBottom:10}}>CREAR PARTIDO</p><label style={S.lbl}>Local</label><select style={S.inp} value={localId} onChange={e=>setLocalId(e.target.value)}><option value="">Seleccionar...</option>{teams.map(t=><option key={t.id} value={t.id}>{t.nombre}</option>)}</select><label style={{...S.lbl,marginTop:8}}>Visitante</label><select style={S.inp} value={visitId} onChange={e=>setVisitId(e.target.value)}><option value="">Seleccionar...</option>{teams.filter(t=>t.id!==localId).map(t=><option key={t.id} value={t.id}>{t.nombre}</option>)}</select><button onClick={()=>{if(localId&&visitId)onCrearPartido(localId,visitId);}} disabled={!localId||!visitId} style={{...S.btnFull,marginTop:12,background:localId&&visitId?"#7c3aed":"#e2e8f0",color:localId&&visitId?"white":"#94a3b8"}}>🏀 INICIAR</button></div>}{ec.length>0&&<div style={{...S.card,marginTop:10}}><div style={{padding:"10px 16px",fontFamily:F.bar,fontWeight:800,fontSize:"0.88rem",color:"#7c3aed",borderBottom:"1px solid #f1f5f9"}}>PROGRAMADOS ({ec.length})</div>{ec.map(m=>{const l=teams.find(t=>t.id===m.localId);const v=teams.find(t=>t.id===m.visitId);return(<button key={m.id} onClick={()=>{setMesaMatch(m);setMesaStep("active");}} style={{width:"100%",padding:"10px 14px",background:"white",border:"none",borderBottom:"1px solid #f1f5f9",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontWeight:700,fontSize:"0.92rem",color:"#1e293b"}}>{l?.nombre} vs {v?.nombre}</span><span style={{fontWeight:900,color:"#7c3aed",fontSize:"1rem"}}>{m.marcadorL||0} - {m.marcadorV||0}</span></button>);})}</div>}</div></div>);}
